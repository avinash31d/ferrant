...
use ferrant::integrations::{IntegrationCapability, IntegrationRegistry};
...
async fn main() -> anyhow::Result<()> {
    ...
    let data_dir = std::env::temp_dir().join("ferrant-advanced-runtime");
    ...
    let report = ingestion
        .ingest(vec![
            Document {
                id: "graph-guide".into(),
                text: "ferrant checkpoints graph supersteps. Failed nodes resume with stable \
    ...
    let tracer = tracer.push(ferrant::OpenTelemetryTracer);
    ...
    let question = "How does ferrant make runs durable and observable?";
