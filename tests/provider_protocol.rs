...
use liteagent::llm::Model;
use liteagent::{Message, StreamEvent, ToolSpec};
...
fn full_json_schema_validation_catches_advanced_constraints() {
    ...
    assert!(liteagent::validate_json(&schema, &json!({"code":"ABC"})).is_ok());
    assert!(liteagent::validate_json(&schema, &json!({"code":"bad","extra":1})).is_err());
}
...
async fn anthropic_structured_output_uses_forced_schema_tool() {
    let response = json!({
        "content":[{"type":"tool_use","id":"tool_1","name":"liteagent_structured_output","input":{"answer":42}}],
    ...
    assert_eq!(
        ...
        Some(&json!("liteagent_structured_output"))
    );
