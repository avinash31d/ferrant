...
async fn no_op(_: liteagent::graph::NodeContext) -> anyhow::Result<NodeOutput> {
...
async fn file_store_survives_graph_reconstruction_and_checks_revisions() {
    let directory =
        std::env::temp_dir().join(format!("liteagent-graph-recovery-{}", uuid::Uuid::new_v4()));
...
async fn bounds_cycles_and_rejects_conflicting_parallel_updates() {
    ...
    assert_eq!(
        ...
        liteagent::graph::GraphFailureKind::StateConflict
    );
}
